...
{
    // TODO (WIP)
    // Based on: https://github.com/bkietz/wnaabi
    // Goal: type_info<bar::baz>::name_tokens(runtime_visitors::stringify_t{}).str
    ...
    /*namespace pretty_function
    {
        ...
        constexpr auto prefix_length_hint = sizeof("char const *wnaabi::anonymous::pretty_function::c_str") - 1;

        constexpr auto strlen_correction = strlen_uncorrected<void>() - (find_offset(c_str<void>() + prefix_length_hint, '\0') + prefix_length_hint);
        static_assert(strlen_correction == strlen_uncorrected<int>() - (find_offset(c_str<int>() + prefix_length_hint, '\0') + prefix_length_hint), "strlen_correction is inconsistent!");
        ...
        // character 'v')
        constexpr auto prefix_length = find_offset(c_str<void>() + prefix_length_hint, 'v') + prefix_length_hint;
        constexpr auto suffix_length = strlen<void>() - (prefix_length + sizeof("void") - 1);
        ...
        template <template <typename...> class T, typename... A>
        constexpr string_literal<templatename_length<T, A...>()> templatename_literal()
        {
            using type = T<A...>;
        ...
        constexpr std::size_t anonymous_scope_length = typename_length<crib>() - (sizeof("wnaabi::") - 1) - (sizeof("pretty_function::crib") - 1);
    ...
    struct TypeInfo
    {
        static_assert(std::is_class<T>::value, "TypeInfo is not specialized for this non-class type");
